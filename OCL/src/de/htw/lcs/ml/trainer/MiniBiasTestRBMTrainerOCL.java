...
public class MiniBiasTestRBMTrainerOCL extends BiasRBMTrainerOCL {

	public MiniBiasTestRBMTrainerOCL(NeuralNetwork rbm) {
		super(rbm);
	}

	public MiniBiasTestRBMTrainerOCL(NeuralNetwork rbmModel, float learningRate, float threashold) {
	...
	{
		int MiniBatchSize = 64;//2048;//4096;
	...
	protected double train(final CLFloatMatrix batchData[], final int MiniBatchSize, final int maxEpochs, final int updateInterval)
	{
		...
		double bestTrainingsError = Double.MAX_VALUE;
		int bestEpoch = 0;
		NeuralNetwork bestNetwork = null;
		final CLFloatMatrix testDataCL = new CLFloatMatrix(testdata.length, numVisible, this.addDataBias(testdata, 0, testdata.length)).enqueue(true);
		final CLFloatMatrix reconstructedTestDataProbs = new CLFloatMatrix(testdata.length, numVisible);
		final CLFloatMatrix posHiddenTestProbs = new CLFloatMatrix(testdata.length, numHidden);
...
//					posHiddenProbs.randomQuantisation();
			...
			System.out.println(epoch+". epoch after "+(System.currentTimeMillis() - start)+" with error "+currentError);

			if (epoch % updateInterval == 0) {

				// positive phase
				super.runVisible(posHiddenTestProbs, testDataCL);
				posHiddenTestProbs.putColVecOnes();

				// reconstruction
				super.runHidden(reconstructedTestDataProbs, posHiddenTestProbs);
				reconstructedTestDataProbs.putColVecOnes();

				// fehler für die Testdaten
				double testError = calcError(testDataCL, reconstructedTestDataProbs);

				// update the ui
				NeuralNetwork nn = new NeuralNetwork(weights.getRows(), weights.getColumns(), weights.dequeue().getElements(), true);
				this.fireTrainingsEvent(nn, epoch, currentError, testError);

				//merke den besten
				if(testError > 0 && testError < bestTestError) {
					bestTrainingsError = currentError;
		...
		if(bestNetwork != null)
			saveWeightsRaw(bestNetwork, Paths.get(String.format("weights/bestoutput_%d_TE%.3f_CVE%.3f.dat", bestEpoch, bestTrainingsError, bestTestError))); // bestTestError
	...
	public TrainerType getTrainerType() {
		return TrainerType.MINIBIASTEST;
	}
