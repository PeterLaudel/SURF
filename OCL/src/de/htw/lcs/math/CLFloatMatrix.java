...
public class CLFloatMatrix {
	...
	private static final String DEFAULT_PROGRAM_NAME = "CLFloatMatrix.cl"; //CLFloatMatrix.cl  //JavaCL BLAS (aus Nativelibs4Java)
	...
	public static CLFloatMatrix ones(final int rows, final int columns) {
		...
		CLFloatMatrix result = new CLFloatMatrix(rows, columns, elements);
		result.enqueue(true);
		return result;
	}
	...
	public float twoStageSumJavaCL() {
		final CLKernel kernel = ocl.getKernel("reduceSum");
		...
		ocl.queue
			.putBarrier()
			.put1DRangeKernel(kernel, 0, lws, lws)
			.putReadBuffer(CLFloatMatrix.sharedBuffer, true);
	...
	public void randomQuantisation(final int seed1, final int seed2)
	{
		...
		ocl.queue
			.putBarrier()
			.put1DRangeKernel(kernel, 0, gws, lws); // TODO 2D Kernel schneller?
	}
	...
	/**
	 * Ab 2048x2048 Matrizen spürbar schneller (20%)
	 *
	 * @param val
	 * @return
	 */
	public CLFloatMatrix mul2d(final float val) {
		final CLKernel kernel = ocl.getKernel("mul_kernel_2d");
		final int length = this.clBuffer.getCLCapacity();
	...
	public CLFloatMatrix emptyCall(final CLFloatMatrix matrixA, final CLFloatMatrix matrixB) {
		final int lws = blockSize;
		final int gws = (int) (lws * Math.ceil(rows /(float)lws));
...
//			.put2DRangeKernel(kernel, 0, 0, this.clRows, this.clColumns, blockDimSize, blockDimSize);
	...
	public CLFloatMatrix dequeueFast(final boolean blockingRead) {
		final float[] wrapped = new float[this.clColumns* this.clRows];
	...
	// Aufruf für JavaCL BLAS (aus Nativelibs4Java)
	private CLFloatMatrix blockMmulJavaCL(final CLKernel kernel, final CLFloatMatrix matrixA, final CLFloatMatrix matrixB, final int M) {
		int DefaultBlockSize = 16; // JavaCL CLDefaultMatrix2D.java

		kernel
			.putArg(matrixA.getCLBuffer())
			.putArg(matrixA.getClColumns())
			.putArg(matrixB.getCLBuffer())
			.putArg(matrixB.getClColumns())
			.putArg(this.clBuffer)
			.putNullArg(DefaultBlockSize * DefaultBlockSize)
			.putNullArg(DefaultBlockSize * DefaultBlockSize);
