...
public class Sorter_RatioDistanceCount implements Sorter {
	...
	public Sorter_RatioDistanceCount(Pic[] pics, String path, int count, float threshold) {
	...
	public void computeDistance(int queryPic, int actPic) {
		...
		List<List<Matches>> match = kdtree.KnnMatching(query.interestPoints, act.interestPoints, 2);
		
		List<Matches> finalMatch = new ArrayList<Matches>();
		
		for(int i = 0; i < match.size(); i++)
		{
			Matches match1 = match.get(i).get(0);
			Matches match2 = match.get(i).get(1);
			float ratio = 0.8f;
